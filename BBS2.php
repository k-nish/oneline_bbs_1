<?php
  ...
  if (isset($_GET['action']) && ($_GET['action'] == 'edit')){
    $id = $POST_['id'];
    $nickname = htmlspecialchars($POST_['nickname']);
    $comment = htmlspecialchars($POST_['comment']);
    // 編集したいデータを取得するSQL文を作成
    $sq = 'SELECT `nickname`,`comment` FROM `posts` WHERE `id`= ?';
    var_dump($sq);
    //SQL文を実行
    $stmt=$dbh->prepare($sq);
    $data[]=$_POST['nickname'];
    ...
    <form action="BBS2.php" method="post">
      <div class="form-group">
            <div class="input-group">
              ...
              <input type="text" name='nickname' value=<?php echo $post['nickname']; ?> class="form-control"
                       id="validate-text" placeholder="nickname" required>
      ...
      <div class="form-group">
            <div class="input-group" data-validate="length" data-length="4">
              <textarea type="text" class="form-control" name='comment' value=<?php echo $post['comment']; ?> id="validate-length" placeholder="comment" required></textarea>
              <span class="input-group-addon danger"><span class="glyphicon glyphicon-remove"></span></span>
            </div>
      </div>
      ...
      <div class="col-md-8 content-margin-top">
        ...
        <article class="timeline-entry">
            ...
            <div class="timeline-entry-inner">
                <a href="BBS2.php?action=edit&id=<?php echo $post['id'];?>">
